...
def analyze_stock(stock_code, stock_name, industry):
    ...
    df['SMA_20'] = talib.SMA(df['Close'], timeperiod=20)  # 計算月均線，假設一個月有20個交易日
    ...
    result_df['Bullish_MA'] = (df['SMA_5'] > df['SMA_10']) & (df['SMA_5'] > df['SMA_20']) & (df['SMA_10'] > df['SMA_20'])  # 判斷是否呈現均線多頭排列
    result_df['Bullish_MA_2'] = (df['SMA_10'] > df['SMA_20']) & (df['SMA_10'] > df['SMA_60']) & (df['SMA_20'] > df['SMA_60'])  # 判斷是否呈現均線多頭排列
    ...
    if not result_df.empty:
        ...
        if last_row['ADX_increase'] and last_row['+DI_greater_than_-DI'] and last_row['Bullish_MA']:
...
def format_matched_stocks_2(matched_stocks):
    matched_stocks_str = ''
    total = 0
    count = 0
    for stock in matched_stocks:
        code = stock[0].split('.')[0]  # 只保留 "." 前面的部分
        matched_stocks_str += code + '\n'
        total += 1
        count += 1
        if count == 150:
...
def main():
    ...
    format_matched_stocks_2(matched_stocks)
